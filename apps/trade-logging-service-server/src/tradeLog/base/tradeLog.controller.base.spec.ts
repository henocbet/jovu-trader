...
const service = {
  ...
  tradeLogs: () => FIND_MANY_RESULT,
  tradeLog: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("TradeLog", () => {
  ...
  test("GET /tradeLogs/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/tradeLogs"}/${nonExistingId}`)
  ...
  test("GET /tradeLogs/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/tradeLogs"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /tradeLogs existing resource", async () => {
