/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { TradeLogWhereInput } from "./TradeLogWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { TradeLogOrderByInput } from "./TradeLogOrderByInput";

@ArgsType()
class TradeLogFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => TradeLogWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => TradeLogWhereInput, { nullable: true })
  @Type(() => TradeLogWhereInput)
  where?: TradeLogWhereInput;

  @ApiProperty({
    required: false,
    type: [TradeLogOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [TradeLogOrderByInput], { nullable: true })
  @Type(() => TradeLogOrderByInput)
  orderBy?: Array<TradeLogOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { TradeLogFindManyArgs as TradeLogFindManyArgs };
